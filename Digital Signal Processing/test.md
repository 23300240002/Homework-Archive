...
- **卷积**：$x_1[n] * x_2[n] \leftrightarrow X_1(e^{j\omega}) X_2(e^{j\omega})$
- **调制**：$x_1[n] x_2[n] \leftrightarrow \frac{1}{2\pi} \int_{-\pi}^{\pi} X_1(e^{j\theta}) X_2(e^{j(\omega-\theta)}) d\theta$
...
- **DFS**：对于周期为 $N$ 的离散序列 $\tilde{x}[n]$，其傅里叶级数为 $\tilde{x}[n] = \frac{1}{N} \sum_{k=0}^{N-1} \tilde{X}[k] e^{j 2\pi k n / N}$，其中 $\tilde{X}[k]$ 是周期也为 $N$ 的离散谱。DFS 是 DFT 的理论基础。
...
- **线性**：$\mathrm{DFT}[a x_1[n] + b x_2[n]] = a X_1[k] + b X_2[k]$

- **圆周移位**：设 $x[(n-m))_N]$ 表示将序列 $x[n]$ 向右循环移位 $m$ 位（模 $N$），则：
...
直接计算一个 $N$ 点 DFT 需要 $N^2$ 次复数乘法和 $N(N-1)$ 次加法。当 $N$ 较大时，计算量惊人。1965 年 Cooley 和 Tukey 提出的快速傅里叶变换（FFT）利用旋转因子的对称性和周期性，将计算量降至 $N \log_2 N$ 量级，极大地推动了数字信号处理的发展。
...
- $k=0$：$X[0] = 1+2+3+4 = 10$
- $k=1$：$X[1] = 1\cdot 1 + 2\cdot (-j)^1 + 3\cdot (-j)^2 + 4\cdot (-j)^3 = 1 - 2j + 3(-1) + 4(j) = 1 - 2j -3 + 4j = -2 + 2j$
- $k=2$：$X[2] = 1 + 2\cdot (-j)^2 + 3\cdot (-j)^4 + 4\cdot (-j)^6 = 1 + 2(-1) + 3(1) + 4(-1) = 1 -2 +3 -4 = -2$
...
- $n=0$：$m=0,1,2,3$，$h[(0-m))_4]$ 分别为 $h[0]=1$，$h[3]=4$，$h[2]=3$，$h[1]=2$，故 $y[0] = 1\cdot1 + 1\cdot4 + 1\cdot3 + 1\cdot2 = 10$
- $n=1$：$h[(1-m))_4]$ 为 $h[1]=2$，$h[0]=1$，$h[3]=4$，$h[2]=3$，$y[1] = 2+1+4+3 = 10$
- $n=2$：$h[(2-m))_4]$ 为 $h[2]=3$，$h[1]=2$，$h[0]=1$，$h[3]=4$，$y[2] = 3+2+1+4 = 10$
- $n=3$：$h[(3-m))_4]$ 为 $h[3]=4$，$h[2]=3$，$h[1]=2$，$h[0]=1$，$y[3] = 4+3+2+1 = 10$

故 $y[n] = \{10,10,10,10\}$。

现在计算 DFT：
- $X[0]=4$，$X[1]=0$，$X[2]=0$，$X[3]=0$（可用公式验证，实际上 $x[n]$ 为常数序列，只有直流分量）
- $H[0]=10$，$H[1]= -2+2j$，$H[2]= -2$，$H[3]= -2-2j$（由例题1结果）

则 $Y[k] = X[k]H[k]$ 得：
- $Y[0]=4\times10=40$
- $Y[1]=0$，$Y[2]=0$，$Y[3]=0$

对 $Y[k]$ 做 IDFT：$y[n] = \frac{1}{4}\sum_{k=0}^{3} Y[k] W_4^{-kn} = \frac{1}{4} \cdot 40 = 10$，与圆周卷积结果一致。验证了时域圆周卷积对应频域乘积。
...
实际上，每个蝶形单元包含两次加减和一次复数乘法。通过这种分解，计算量从 $4^2=16$ 次复数乘法减少为 $2$ 次（实际为 $\frac{N}{2}\log_2 N = 4$ 次，但 $N=4$ 时仅需 $4$ 次，远小于 $16$）。