# Homework-Archive
各类课程作业与资料归档

## Algebraic Structures and Mathematical Logic → 代数结构与数理逻辑 许扬

## Computer Network → 计算机网络(H) 徐扬

## Theory of Computation → 计算理论基础 李元

## Probability and Mathematical Statistics → 概率论与数理统计 郭跃飞

## Discussion on Films and British and American Culture → 影视与英美文化讨论 苗伟

## Artificial Intelligence → 人工智能（H） 郑骁庆（暂未公开，请私聊）

## Object-Oriented Programming → 面向对象程序设计 周雅倩

## Digital Signal Processing → 数字信号处理 薛向阳

## Computer Graphics‌ → 计算机图形学 颜波（暂未公开，请私聊）
